...
function PaymentNotification({navigation, route}) {
  ...
  useEffect(() => {
    ...
    // AsyncStorage.getItem('activeUserProfile').then(profile => {
    //   let myProfile = JSON.parse(profile);
    //   paymentData.current.studentId = myProfile?.studentId;
    // });
  }, [navigation]);
        ...
        const submitPaymentNotification = async () => {
          ...
          if (gestureState.dx >= 128) {
            ...
            if (
              ...
              formData.append('narration', payNarration);
              formData.append('paymentEvidence', {
                uri: uri,
              ...
              await axios
                .post(
                  'http://172.20.10.8:3001/paymentNotificationTwo/sendnotification',
                  formData,
                  config,
                )
                .then(res => {
